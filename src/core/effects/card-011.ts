...
function getCard011Period(speed: number): number {
...
export default createEffectCardSource({
  ...
  load(game, player, level) {
    log(`Setting up Card 011 for ${player.name}`);
    game.on(EventType.Start, EventPriority.Post, () => {
      let elapsed = 0;
      let period = getCard011Period(player.speedStacks);
      ...
      const cleanup = createTick(() => {
        ...
        if (elapsed >= period) {
          elapsed -= period;
          period = getCard011Period(player.speedStacks);
