...
function getCard003Period(speed: number): number {
...
export default createEffectCardSource({
  ...
  getDescription(level) {
    return [
      'Periodically gains ',
      level,
      ' stacks of Armor. Period ranges from',
      5,
      ' seconds to ',
      0.2,
      ' seconds based on ',
  ...
  load(game, player, level) {
    log(`Setting up Card 003 for ${player.name}`);
    game.on(EventType.Start, EventPriority.Post, () => {
      let elapsed = 0;
      let period = getCard003Period(player.speedStacks);
      ...
      const cleanup = createTick(() => {
        ...
        if (elapsed >= period) {
          elapsed -= period;
          period = getCard003Period(player.speedStacks);

          game.triggerBuff(EventType.AddArmor, player, level);
        }
      });
