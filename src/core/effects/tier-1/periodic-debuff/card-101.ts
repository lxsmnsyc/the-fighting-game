...
function getCard101Period(speed: number): number {
...
export default createEffectCardSource({
  ...
  load(game, player, level) {
    log(`Setting up Card 101 for ${player.name}`);
    game.on(EventType.Start, EventPriority.Post, () => {
      let elapsed = 0;
      let period = getCard101Period(player.speedStacks);
      ...
      const cleanup = createTick(() => {
        ...
        if (elapsed >= period) {
          elapsed -= period;
          period = getCard101Period(player.speedStacks);
