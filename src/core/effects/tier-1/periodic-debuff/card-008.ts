...
function getCard008Period(speed: number): number {
...
export default createEffectCardSource({
  ...
  getDescription(level) {
    return [
      ...
      level,
      ' stacks of Armor Decay to the enemy. Period ranges from',
      5,
      ' seconds to ',
      0.2,
      ' seconds based on ',
  ...
  load(game, player, level) {
    log(`Setting up Card 008 for ${player.name}`);
    game.on(EventType.Start, EventPriority.Post, () => {
      let elapsed = 0;
      let period = getCard008Period(player.speedStacks);
      ...
      const cleanup = createTick(() => {
        ...
        if (elapsed >= period) {
          elapsed -= period;
          period = getCard008Period(player.speedStacks);
