...
export default function RunStatus(props: RunStatusProps): JSX.Element {
  ...
  const maxLife = read(() => props.game.checkMaxLife());
  ...
  return (
    <div class="flex flex-col gap-1">
      ...
      <div class="flex gap-1 text-lg leading-none">
        <For each={Array.from({ length: maxLife() }, (_, index) => index)}>
          {(index) => (
            <span data-testid="life" class={index < lives() ? 'text-rose-500' : 'text-zinc-700'}>
              ♥
            </span>
          )}
        </For>
